...
1. **header 三行制**：站名（display 400）→ hook「今晚，翻開一本就好。」（中）→ 一行小統計「已上架 N 本・每晚 19:00 更新」。刪除大數字統計與小副標，節奏只降不跳。
2. **今晚主打卡**：最新一本（date=今天或最近）放大版目錄卡——「今晚的書」accent 小 chip＋迷你書封（96px 寬）＋書名＋作者＋完整 hook＋實心膠囊「開始閱讀」。
3. **目錄卡（過往藏書）**：橫式卡（--card 底、1px --rule 框、radius 10px、padding 20px、flex gap 16px）。左＝迷你書封 72px 寬 2:3（主題色＋左緣 4px 深一階書脊＋內縮 4px 的 1px 髮絲裝幀框＋書名小字置中，不塞作者主題）；右＝索引行小字「No.001・自我成長」→ 書名（serif 700 1.25rem）→ 作者小字 → hook 最多兩行截斷 → 底行「M/D・閱讀 N 分鐘」＋accent「開始閱讀 →」。
